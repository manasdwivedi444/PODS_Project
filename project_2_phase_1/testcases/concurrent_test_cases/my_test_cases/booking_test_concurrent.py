...
def main():
    ...
    if(show["seats_available"]!=30):
        print("failed seat update after delete all")
        return
...
def test(thread_id):
    ...
    user_response=create_user("abc"+str(thread_id),"abc"+str(thread_id)+"@xyz.com")
    user=user_response.json()
    userid1=user["id"]
    user_response=create_user("abcd"+str(thread_id),"abcd"+str(thread_id)+"@xyz.com")
    user=user_response.json()
    userid2=user["id"]
    user_response=create_user("abcde"+str(thread_id),"abcde"+str(thread_id)+"@xyz.com")
    user=user_response.json()
    userid3=user["id"]
    ...
    if(response.status_code!=200):
        print("failed get theatres for thread-"+str(thread_id))
        return
    print("printing theatres for thread-"+str(thread_id))
    print(response.json())

    print("passed test for getting theatres for thread-"+str(thread_id))

    #get shows with theatreid
    response = get_shows_with_theatreid(2)
    if(response.status_code!=200):
        print("failed to get shows with theaterid for thread-"+str(thread_id))
        return
    print("printing shows with theatre id 2 for thread-"+str(thread_id))
    print(response.json())

    print("passed test for getting shows with theatre_id for thread-"+str(thread_id))

    #get shows with non-exixting theatreid
    response = get_shows_with_theatreid(70)
    if(response.status_code!=404):
        print("failed to get shows with non-existing theaterid for thread-"+str(thread_id))
        return
    
    print("passed test for getting shows with non-existing theatre_id for thread-"+str(thread_id))

     #get shows with showid
    response = get_shows_with_showid(2)
    if(response.status_code!=200):
        print("failed to get shows with showid for thread-"+str(thread_id))
        return
    print(response.json())

    print("passed test for getting shows with show_id for thread-"+str(thread_id))

    #get shows with non-exixting showid
    response = get_shows_with_showid(70)
    if(response.status_code!=404):
        print("failed to get shows with non-existing showid for thread-"+str(thread_id))
        return

    print("passed test for getting shows with non-existing show_id for thread-"+str(thread_id))

    #get booking for user with no bookings
    response = get_bookings_with_userid(userid1)
    if(response.status_code!=200):
        print("failed to get booking"+str(thread_id))
        return     
    if(len(response.json())!=0):
        print("failed to get booking for user with no bookings for thread-"+str(thread_id))
        return

    print("passed test for getting booking for user with no bookings for thread-"+str(thread_id))

    #create booking for invalid show
    response = create_booking(userid1,80,1)
    if(response.status_code!=400):
        print("failed invalid show booking for thread-"+str(thread_id))
        return
    
    print("passed test for create booking for invalid show for thread-"+str(thread_id))
    
    #create booking for invalid user
    response = create_booking(1000,2,1)
    if(response.status_code!=400):
        print("failed invalid user booking for thread-"+str(thread_id))
        return
    
    print("passed test for create booking for invalid user for thread-"+str(thread_id))
    ...
    if(response.status_code!=400):
        print("failed booking less money for thread-"+str(thread_id))
        return

    print("passed test for create booking with less money for thread-"+str(thread_id))

    #create booking with high no of seats
    response=create_booking(userid2,2,40)
    if(response.status_code!=400):
        print("failed booking high no. of seats for thread-"+str(thread_id))
        return

    print("passed test for create booking with high no of seats for thread-"+str(thread_id))

    #create a booking
    response=create_booking(userid2,2,1)
    if(response.status_code!=200):
        print("failed to create booking for thread-"+str(thread_id))
        return
    
    print("passes test for creating a booking for thread-"+str(thread_id))

    #check balance and seats after booking
    response=get_shows_with_showid(2)
    show=response.json()
    if(show["seats_available"]>29):
        print("failed seat update for thread-"+str(thread_id))
        return
    response=get_wallet(userid2)
    wallet=response.json()
    if(wallet["balance"]!=(10000-55)):
        print("failed update wallet for thread-"+str(thread_id))
        return

    print("passed test for checking balance and seats after booking for thread-"+str(thread_id))

    #print show 1 and 3
    response=get_shows_with_showid(1)
    show=response.json()
    print("printing shows with show id 1 and 3 for thread-"+str(thread_id))
    print(show)
    ...
    if(response.status_code!=200):
        print("fail to get booking for thread-"+str(thread_id))
    booking=response.json()
    print("printing booking for user2 for thread-"+str(thread_id))
    print(booking)

    print("passed test for get booking for thread-"+str(thread_id))

    #delete booking using userid and showid
    response=delete_booking_with_userid_showid(userid2,2)
    if(response.status_code!=200):
        print("fail to delete booking for thread-"+str(thread_id))
        return
    
    print("passed test for delete booking using userid and showid for thread-"+str(thread_id))
    
    #check available seats and wallets balance
    response=get_shows_with_showid(2)
    show=response.json()
    if(show["seats_available"]>28):
        print("failed seat update after delete for thread-"+str(thread_id)+str(show["seats_available"]))
        return
    response=get_wallet(userid2)
    wallet=response.json()
    if(wallet["balance"]!=(10000-60)):
        print("failed update wallet after delete for thread-"+str(thread_id))
        return
    
    print("passed test to update seats and balance after delete for thread-"+str(thread_id))
    ...
    print("passed test for deleting booking with userrid for thread-"+str(thread_id))

    #check available seats and wallets balance
    response=get_shows_with_showid(2)
    show=response.json()
    if(show["seats_available"]>30):
        print("failed seat update after delete for thread-"+str(thread_id))
        return
    ...
    print("passed test to delete non-existing booking for thread-"+str(thread_id))
    ...
    booking=response.json()
    if(len(booking)==0):
        print("failed to get bookings for thread-"+str(thread_id))
        return
    ...
    booking=response.json()
    if(len(booking)!=0):
        print("failed to remove booking by delete user for thread-"+str(thread_id))
        return

    print("passed test to remove booking after user is deleted for thread-"+str(thread_id))
