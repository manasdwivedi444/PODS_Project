...
public class BookingRoutes {
	   ...
	   private CompletionStage<List<Booking>> getBookings(int user_id){
	   ...
	   public Route bookingRoutes() {
			  return concat( pathEnd( () -> 
			                     concat( post(() ->  entity( Jackson.unmarshaller(Booking.class), booking ->
			                                             onSuccess(createBooking(booking) , newbooking -> { if(booking.getId()==-1) return complete(StatusCodes.BAD_REQUEST, newbooking, Jackson.marshaller());
					         ...
					         pathPrefix("users",() -> 
					              pathPrefix( integerSegment(), user_id ->
					                  concat( pathEnd (() ->
					                              concat( get (() -> onSuccess(getBookings(user_id), bookings -> complete(StatusCodes.OK, bookings, Jackson.marshaller()))),
