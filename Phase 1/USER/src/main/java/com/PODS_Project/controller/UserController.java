...
public class UserController {
	...
	// Establishes connection with bookingRepository and making request to delete bookings of user with Id equal to user_id.
				URL url = new URL("http://host.docker.internal:8081/bookings/users/"+id);  
    ...
    // Establishes connection with walletRepository and making request to delete wallet of user with Id equal to user_id.
				URL url = new URL("http://host.docker.internal:8082/wallets/"+id); 
				HttpURLConnection connection = (HttpURLConnection) url.openConnection();
				connection.setRequestMethod("DELETE");
				int responseCode = connection.getResponseCode();
	...
	// if 1st is situationn, code execution will never reach this point because of earlier if statement.
	// if 2nd is situation, we dont have to do anything as wallet user with Id equal to user_id already doesn't exist.
				if(responseCode!=HttpURLConnection.HTTP_OK) {
	...
	// connects with WALLET microservice and deletes all wallets from walletRepository(as there shouln't be any wallet if there is no users)
		try {
			URL url = new URL("http://host.docker.internal:8082/wallets");
			HttpURLConnection connection = (HttpURLConnection) url.openConnection();
			connection.setRequestMethod("DELETE");
			int responseCode = connection.getResponseCode();
